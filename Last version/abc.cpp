...
ABC::ABC(
		...
		vector<double> Cmin,
		vector<double> Cmax,
		double (*func)(vector<vector<double>> X, vector<int> y, vector<double> W)
	){
...
void ABC::init(int index){
	...
	f[index] = object_func(X, Y, solution);
...
void ABC::SendEmployedBees(){
	...
	for (i = 0 ;i<FoodNumber ;i++){
		...
		ObjValSol = object_func(X, Y, solution);
...
void ABC::fit(vector<vector<double>> X, vector<int> Y){
	...
	for (cycle = 0; cycle<maxCycle; cycle++){
		...
		MemorizeBestSource();
		SendScoutBees();
		cout <<cycle<< "- GlobalMin = " << GlobalMin << endl;
	}
	for (j = 0; j < D; j++)
